//---------------CUDA algorithm[Compact_inverse] executated by OCCA2---------------------
//--------------------------by Xin at GeonX-----------------------------------
...
int main(int argc, char **argv){
  ...
  occa::kernel kernel_compact_bool_inverse = device.buildKernelFromSource(GeneralTools::get_occaKernels_path("Compact_inverse.occa"), "Compact_bool_inverse", args);
  occa::kernel kernel_scan_HS = device.buildKernelFromSource(GeneralTools::get_occaKernels_path("Scan.occa"), "Scan_add_HS", args);
  occa::kernel kernel_compact_map_inverse = device.buildKernelFromSource(GeneralTools::get_occaKernels_path("Compact_inverse.occa"), "Compact_map_inverse", args);
  ...
  */
  ...
  occa::memory b_inverse_gpu = device.malloc(filter_sz_inverse,b_inverse_cpu);
  occa::memory b_tp_gpu = device.malloc(blocksPerGrid*sizeof(int));
  //---------------------------------------------------
  
  

  kernel_compact_bool_inverse(
    filter_gpu,
    filter_size,
    bool_gpu
    );
  ...
  b_inverse_gpu.copyTo(b_inverse_cpu);
  if(verbose)
  {
    cout << "b_inverse_cpu: "; 
    for(int i =0; i< N-filter_size; ++i) cout << b_inverse_cpu[i] <<", ";
    cout << endl;
  }
